...
𝑖
i
: The unique index of the thread/element (0 to 
𝑁
−
1
N−1
...
𝑏
b
: The bit value of the element at the current position (0 or 1).

onesBefore: How many elements before index 
𝑖
i
 have a bit value of 1. (Calculated via Exclusive Scan).
...
onesBefore
dest=i−onesBefore
	The new index is the original index minus the space taken up by 1s that used to be in front of it.
If bit 
𝑏
=
=
...
+
onesBefore
dest=totalZeros+onesBefore
	The element must go after all the zeros. Its offset is determined by how many 1s were before it.
...
Exclusive Scan (Find onesBefore):

Input: [1, 0, 0, 1]

Scan Result: [0, 1, 1, 1]

Note: The total sum (2) is stored at the end. 
→
→
 totalOnes = 2, totalZeros = 2.

Calculate Destination:

Thread 0 (Val 5, bit 1): totalZeros + onesBefore 
→
→
...
Thread 3 (Val 3, bit 1): totalZeros + onesBefore 
→
→
...
Perform the Radix Sort/Shuffle inside Shared Memory.

Once sorted locally, the 0s are contiguous and the 1s are contiguous within the block.

The block writes its chunk of 0s to the global 0-bucket and its chunk of 1s to the global 1-bucket in coalesced, sequential writes.
...
Local Sort: Break array into small tiles. Each thread block sorts its tile independently (using a fast local sort like bitonic or radix).

Merge Tree: Iteratively merge pairs of sorted lists until one big list remains.
...
Thread Block 0 sorts [8, 2] 
→
→
 [2, 8]

Thread Block 1 sorts [9, 4] 
→
→
 [4, 9]

Thread Block 2 sorts [5, 3] 
→
→
 [3, 5]

Thread Block 3 sorts [1, 6] 
→
→
 [1, 6]
...
Co-Rank Search: Find start for Rank 0 (
𝑖
=
0
,
...
Co-Rank Search: Find start for Rank 4.
